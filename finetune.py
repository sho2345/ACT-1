from config.config import POLICY_CONFIG, TASK_CONFIG, TRAIN_CONFIG, FINETUNE_CONFIG # must import first
...
parser.add_argument('--checkpoint', type=str, default=None, help="Path to a pre-trained checkpoint for fine-tuning")
...
def finetune(train_dataloader, val_dataloader, policy_config, checkpoint):
    # load policy
    policy = make_policy(policy_config['policy_class'], policy_config)
    policy.to(device)
    ...
    ckpt_dir = os.path.join(checkpoint_dir, "finetune")
    if not os.path.exists(ckpt_dir):  # パスが存在するか確認
...
if __name__ == '__main__':
   ...
   # number of training episodes
    ...
    # train
    if args.checkpoint:
        finetune(train_dataloader, val_dataloader, policy_config, args.checkpoint)
    else:
